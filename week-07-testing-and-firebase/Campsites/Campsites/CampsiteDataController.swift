...
class CampsiteDataController {
    ...
    lazy var responseHandler: (Data?, URLResponse?, Error?) -> Void = {(_ data: Data?, _ response: URLResponse?, _ error: Error?) -> Void in
        ...
        guard statusCode == 200 else {
            ...
            //pass empty array to update closure
            self.onDataUpdate?([Campsite]())
            return
        }
        //download successful
    ...
    func loadJson(stateCode: String) {
        ...
        let dataTask = session.dataTask(with: url, completionHandler: responseHandler)
